...
Fapi_Provision_Finish(FAPI_CONTEXT *context)
{
    ...
    switch (context->state) {
        ...
        statecase(context->state, PROVISION_WRITE_HIERARCHIES);
            ...
            if (command->hierarchies[command->path_idx].handle == ESYS_TR_RH_OWNER)
                hierarchy = hierarchy_hs;
            else if (command->hierarchies[command->path_idx].handle == ESYS_TR_RH_ENDORSEMENT)
                hierarchy = hierarchy_he;
            else if (command->hierarchies[command->path_idx].handle == ESYS_TR_RH_LOCKOUT)
                hierarchy = hierarchy_lockout;
            else
                goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "Invalid hierarchy list.",
                           error_cleanup);

            r = ifapi_keystore_store_async(&context->keystore, &context->io,
                                           command->pathlist[command->path_idx], hierarchy);
